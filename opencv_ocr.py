...
class opencv_ocr(object):
    ...
    def _svm_classify_one_char(self, model, img):
        ...
        bin_norm, _ = self.__scall_and_border_image(
            img, (CHAR_SIZE, CHAR_SIZE))
        ...
        for _split in split_list:
            ...
            if _h1 > size_limit[1] or _w1 > 1.2 * _h1:
                ...
                if len(__split) > 1 and __split[0] != [] and __split[1] != []:
                    _idx = self._split_add_iter(
                        _idx, _index, __split, image_list, size_limit)
                continue
    ...
    def _correct_char_image(self, image_list, size_limit):
        ...
        for _index, _img in enumerate(copy.copy(image_list)):
            ...
            # 粗分割
                ...
                _idx = self._split_add_iter(
                    _idx, _index, _list_img, image_list, size_limit)
            # 细分割判断
    ...
    def _character_location(self, image, Precise=True):
        ...
        if Precise:
            ...
            # Get largest counter, (counotour == Irregular figure)
            contour = max(contours, key=cv2.contourArea)
            #  Get the largest rectangle containing the contour, this rectangle has a rotation angle
            rect = cv2.minAreaRect(contour)
            # Get the coordinates of the 4 corners of that rectangle might be inclined
            ...
            # Get the len and width of the rectangle
            rect_w, rect_h = int(np.sqrt((x3 - x2)**2 + (y3 - y2)**2)
                                 ), int(np.sqrt((x2 - x1)**2 + (y2 - y1)**2))
            # get the matrix by affine transformation
            src = np.float32([[x1, y1], [x2, y2], [x3, y3]])
            dst = np.float32([[0, rect_h], [0, 0], [rect_w, 0]])
            mat = cv2.getAffineTransform(src, dst)
            # With this function,
            # the text area can be corrected to the horizontal direction
            img_rot = cv2.warpAffine(img_binary, mat, (w, h))
            # Binarize (if that's a word) again, we must ensure that it is white on a black background
            _, img_rot = cv2.threshold(
                img_rot, 0, 255, cv2.THRESH_OTSU)
            # Intercept the smallest rectangular area
    ...
    def deskew(self, img):
        ...
        img = cv2.warpAffine(img, M, (CHAR_SIZE, CHAR_SIZE),
    ...
    def preprocess_hog(self, digits):
        samples = []
        for img in digits:
            # calculate the gradient val in both directions
            gx = cv2.Sobel(img, cv2.CV_32F, 1, 0)
            gy = cv2.Sobel(img, cv2.CV_32F, 0, 1)
            # Calc the amplitude and the direction of the gradient
            mag, ang = cv2.cartToPolar(gx, gy)
            # divide 360 (2*PI) into 16 bins
            bin_n = 16
            bin = np.int32(bin_n*ang/(2*np.pi))
            bin_cells = bin[:10, :10], bin[10:,
                                           :10], bin[:10, 10:], bin[10:, 10:]
    ...
    def __scall_and_border_image(self, src_image, target_size):
        ...
        if _max == _ratio_image:
            ...
            __h = target_size[1]
        _src_image = cv2.resize(src_image, (__w, __h),
        ...
        _src_image = cv2.copyMakeBorder(
            _src_image, _top, _bottom, _left, _right, cv2.BORDER_CONSTANT, value=0)
