...
class KNearest(StatModel):
    ...
    def predict(self, samples):
        retval, results, neigh_resp, dists = self.model.find_nearest(
            samples, self.k)
...
def load_chars(work_path):
    ...
    for fpathe, dirs, fs in os.walk(work_path):
        for f in fs:
            ...
            ch_img, _ = __scall_and_border_image(
                ch_img, (CHAR_SIZE, CHAR_SIZE))
...
if __name__ == '__main__':
    # Read the character pictures used for training,
    # where the characters at the end of the name are used as labels
    # for supervised learning
    # digits, labels = load_chars('%s/char_good/'%os.getcwd())
    print 'preprocessing...digits=%d,labels=%d' % (len(digits), len(labels))
    # shuffle digits (scramble all chars)
    ...
    if 0:  # Take the first 90% of the training and the last 10% of the test,
